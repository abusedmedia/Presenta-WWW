...
export default {
  ...
  head () {
        return {
            title: 'The Document Generation Blog'
        }
  },
  async asyncData(){
  ...
  methods:{
      getImage(post){
          let img = `https://cache.presenta.workers.dev/zGywhb2oJn:ah8cqUtoW?&term=${parseInt(Math.random()*100)}&title=${post.title}&subtitle=${post.date}`
